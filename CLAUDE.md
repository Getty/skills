...
content, silently. **Edit any existing SKILL.md only with an in-place
truncating write, and verify:**
...
- Groups added/renamed → regenerate manifests: `manage-skills package`, then
  `claude plugin validate .` (a root-CLAUDE.md warning is expected; don't use
  `--strict`).
